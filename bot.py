...
def upload(update: Update, context: CallbackContext) -> None:
    ...
    if len(context.args) < 3:
        update.message.reply_text("⚙️ Usage: /upload [image_url] [character_name] [rarity]")
        return
...
# Stats command (owner-only) to view bot statistics
def stats(update: Update, context: CallbackContext) -> None:
    if update.message.from_user.id != OWNER_ID:
        update.message.reply_text("🚫 You don't have permission to use this command.")
        return

    update.message.reply_text("📊 Here are the bot's statistics:\n- Total Characters: [placeholder]\n- Total Users: [placeholder]")
